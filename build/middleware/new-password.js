...
export function show(options, middlewares) {
...
var validateInput = function (req, res, next) {
    return settings.validatePassword(check(settings.passwordField))(req, res, next);
};
...
var savePassword = function (req, res, next) {
    ...
    changePassword(identity, password).then(() => {
        let code = getCode(req);
...
export function save(options, middlewares) {
    settings = _.merge(defaultSettings, options || {});
    ...
    ret.push(checkCode, validateInput, validateRequest, checkEqualPassword, savePassword);
