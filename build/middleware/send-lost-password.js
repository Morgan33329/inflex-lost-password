...
import { generateHash, send } from './../services/hash';
...
var checkEmailExists = function (req, res, next) {
    ...
    repository("account").findOneByAccount(to).then(account => {
        if (!account) {
        ...
        repository("password").hasPassword(account.identity_id).then(has => {
            if (has) {
                ...
                if (settings.noPassword) settings.noPassword(req, res);else settings.emailNotExists(req, res);
            }
        });
    }).catch(err => {
        console.log(err);
...
export default function (options, middlewares) {
    ...
    ret.push(validateEmail, isValidRequest, checkEmailExists, sendEmail);
